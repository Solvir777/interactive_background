...
fn main() {
    ...
    let mut initial_data = UBO { time: 0.0, position: [0.5, 0.5] , start_time: 0.0};
    ...
    let mut pipeline = window_size_dependent_setup(
        memory_allocator.clone(),
        cs.clone(),
    );
        ...
        match event {
            ...
            Event::RedrawEventsCleared => {
                ...
                if recreate_swapchain {
                    ...
                    let new_pipeline = window_size_dependent_setup(
                        memory_allocator.clone(),
                        cs.clone(),
                    );
                ...
                let uniform_time = SystemTime::now().duration_since(start_time).unwrap().as_secs_f32();
                initial_data.time = uniform_time;
                let img = &images[image_index as usize];
                let framebuffer_view = ImageView::new(img.clone(), ImageViewCreateInfo::from_image(&*img)).unwrap();
                ...
                let set = PersistentDescriptorSet::new(
                    ...
                    layout.clone(),
                    [
                        WriteDescriptorSet::image_view(0, framebuffer_view),
                    ],
                    [],
                )
                    .unwrap();
...
fn window_size_dependent_setup(
    ...
    let pipeline = {
        let stages = PipelineShaderStageCreateInfo::new(cs);
